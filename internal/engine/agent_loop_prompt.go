...
func buildNativeMetaToolInstructions(backend []tools.ToolSpec) string {
	lines := []string{
		...
		"For multi-provider/model work, use orchestrate DAG stages with `model` for explicit routing or `race` for 2-3 provider-profile candidates; reserve race for high-value synthesis or flaky providers because it costs N times more.",
		"Every tool_call/tool_batch_call MUST include `_reason`. Put it at the wrapper level for the overall intent, and inside each backend args object when an inner call has a sharper reason; batch calls inherit the wrapper reason only as a fallback.",
