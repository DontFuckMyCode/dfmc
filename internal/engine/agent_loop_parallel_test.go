...
func TestExecuteToolCallsParallel_RunsConcurrently(t *testing.T) {
	...
	start := time.Now()
	results := eng.executeToolCallsParallel(context.Background(), calls, 4, "agent", nil, nil, nil, nil)
...
func TestExecuteToolCallsParallel_PreservesOrder(t *testing.T) {
	...
	results := eng.executeToolCallsParallel(context.Background(), calls, 3, "agent", nil, nil, nil, nil)
	for i, r := range results {
		if r.Index != i {
...
func TestExecuteToolCallsParallel_BatchSizeZeroFallsBackToSequential(t *testing.T) {
	...
	results := eng.executeToolCallsParallel(context.Background(), calls, 0, "agent", nil, nil, nil, nil)
	if len(results) != 2 {
