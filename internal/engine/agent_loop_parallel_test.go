...
func TestExecuteToolCallsParallel_RunsConcurrently(t *testing.T) {
	...
	start := time.Now()
	results := eng.executeToolCallsParallel(context.Background(), calls, 4)
...
func TestExecuteToolCallsParallel_PreservesOrder(t *testing.T) {
	...
	results := eng.executeToolCallsParallel(context.Background(), calls, 3)
	for i, r := range results {
		if r.Index != i {
...
func TestExecuteToolCallsParallel_BatchSizeZeroFallsBackToSequential(t *testing.T) {
	...
	results := eng.executeToolCallsParallel(context.Background(), calls, 0)
	if len(results) != 2 {
