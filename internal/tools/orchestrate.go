...
- When you actually need the intermediate tool output visible to the main loop. Sub-agents return summaries only.`,
		...
		Args: []Arg{
			...
			{Name: "stages", Type: ArgArray, Description: "Explicit DAG: array of {id, task, depends_on?, title?, hint?}. When provided, skips the text splitter and runs stages in topological layers."},
