...
// defaults_catalog.go — the static command manifest. Sibling to
// defaults.go which keeps DefaultRegistry construction + the sync.Once
// cache. Group order is the canonical help-output order: Query →
// Analyze → Project → Memory → Tools → Config → Server → System.
