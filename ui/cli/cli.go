...
//
// When adding a new subcommand, wire it into cli_dispatch.go and put its
// implementation in whichever sibling file matches its concern.
...
func Run(ctx context.Context, eng *engine.Engine, args []string, version string) int {
	...
	if code, ok := dispatchCommand(ctx, eng, cmd, cmdArgs, opts, version); ok {
		return code
	}
