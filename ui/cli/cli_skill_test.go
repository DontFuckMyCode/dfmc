...
func TestDiscoverSkillsIncludesProjectCustom(t *testing.T) {
	...
	// SystemInstruction strips the {input} placeholder for use as the
	// system message; the live skill body it returns must still carry
	// the prompt skeleton.
	if !strings.Contains(found.Prompt, "Review this carefully:") {
		t.Fatalf("project skill prompt skeleton missing: %s", found.Prompt)
	}
}
