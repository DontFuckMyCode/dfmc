...
// slash_picker.go — slash-command catalog, autocomplete, and per-tool
// argument suggestion machinery.
//
// Lifted out of the 10K-line tui.go god file (REPORT.md C1) so the
// "what /should I type next" surface lives in one obvious place.
// Every method continues to live on `Model` — no behaviour change,
// no new abstractions. The chat key handler still calls these via
// buildChatSuggestionState/handleChatKey.
//
...
//                        slashSuggestionsForToken, slashCommandCatalog,
//                        slashTemplateOverrides)
