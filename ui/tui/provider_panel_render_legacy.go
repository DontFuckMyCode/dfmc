...
// provider_panel_render_legacy.go — pre-V2 inline list view + the
// numbered-row / extended-detail formatters. The V2 renderer in
// provider_panel_render.go (renderProviderListViewV2 + sibling cards)
// is the default path; this legacy view is still wired in because the
// inline action menu (m.providers.menuActive) needs the older shape
// where rows + detail + menu sit in one column. When the menu opens
// over a V2 layout, we fall back here so the old keystrokes still
// work without re-implementing menu rendering on the V2 cards.
...
func (m Model) renderProviderListViewLegacy(width int) string {
	...
	for i, row := range rows[scroll:] {
		...
		line := formatProviderRowNumbered(row, idx+1, selected, fallbackPos, width-2)
		// Phase I item 1 — append probe chip when the user has run a
		// test-connection on this row. The chip already carries its own
		// styling (ok/probing/failed) so we just space-join.
		if chip := m.providerProbeChip(row.Name); chip != "" {
			line += "  " + chip
		}
