...
- **Caps**: `max_tool_steps` (default 60), `max_tool_tokens` (default 250000), `max_tool_result_chars` (32K), `max_tool_result_data_chars` (12K), `parallel_batch_size` (4), `meta_call_budget` (64 — cumulative backend calls per turn dispatched via `tool_call`/`tool_batch_call`), `meta_depth_limit` (4 — meta-inside-meta nesting ceiling).
