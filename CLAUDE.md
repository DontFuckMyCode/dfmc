...
- [ui/web/server.go](ui/web/server.go) (~285 lines) — `New`, `Handler`, lifecycle (`Close` releases the rate-limiter gc goroutine). HTTP+SSE on port 7777 (`dfmc serve`). Handlers split by domain: [server_status.go](ui/web/server_status.go), [server_chat.go](ui/web/server_chat.go), [server_context.go](ui/web/server_context.go), [server_tools_skills.go](ui/web/server_tools_skills.go), [server_conversation.go](ui/web/server_conversation.go), [server_workspace.go](ui/web/server_workspace.go), [server_files.go](ui/web/server_files.go), [server_drive.go](ui/web/server_drive.go) (Drive Cockpit), [server_task.go](ui/web/server_task.go) (task store CRUD), [server_admin.go](ui/web/server_admin.go) (scan/doctor/hooks/config). Workbench HTML lives in [ui/web/static/index.html](ui/web/static/index.html) and is loaded via `//go:embed`.
